...
namespace SolRIA.SaftAnalyser
{
	public class OpenedFileInstance
	{
		...
		void ValidateInvoiceLine(SourceDocumentsSalesInvoicesInvoiceLine line, string supPk, string invoiceNo)
		{
			...
			int numCasasDecimais = 6;// Workspace.Instance.Config.NumCasasDecimaisValidacoes;
			bool incidendia = Math.Round(line.UnitPrice * line.Quantity, numCasasDecimais, MidpointRounding.AwayFromZero) - Math.Round(line.Item, numCasasDecimais, MidpointRounding.AwayFromZero) == 0;
			if (incidendia == false)
				MensagensErro.Add(new Error { Value = line.Item.ToString(), Field = line.ItemElementName.ToString(), TypeofError = typeof(SourceDocumentsSalesInvoicesInvoice), Description = string.Format("Total de linha diferente do valor unitário * quantidade. Documento: {0}", line.InvoiceNo), UID = line.Pk, SupUID = supPk });
		}
